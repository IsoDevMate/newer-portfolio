...
export const ExperienceSection = () => {
    const [selectedCompany, setSelectedCompany] = React.useState('BeeMultiscents');
    const experiences = {
    ...
    return (<AnimationWrapper className="py-24 px-6 bg-gradient-to-b from-gray-900 to-gray-800" threshold={0.2}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-8 max-w-5xl mx-auto">
          ...
          <motion.div variants={childVariants} className="md:w-48">
            {companies.map((company) => (<button key={company} onClick={() => setSelectedCompany(company)} className={`w-full text-left p-4 border-l-2 transition-all duration-300 hover:bg-gray-700/50 hover:text-blue-400
          ...
          {/* Experience Details */}
          <motion.div variants={childVariants} className="flex-1 bg-gray-800/50 p-8 rounded-lg border border-gray-700/30 backdrop-blur-sm">
            {experiences[selectedCompany] && (<>
                ...
                <p className="text-gray-400 mb-6">
                  ...
                  {experiences[selectedCompany].period && (<span className="block mt-1">{experiences[selectedCompany].period}</span>)}
                </p>
                <ul className="space-y-4">
                  {experiences[selectedCompany].duties.map((duty, index) => (<li key={index} className="flex items-start gap-3">
