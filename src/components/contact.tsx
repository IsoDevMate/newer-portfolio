...
import { sendContactForm, ActionResponse } from './actions';
import { Modal } from './Modal';
...
const SubmitButton = () => {
  ...
  return (
    <button
      type="submit"
      className="w-full px-8 py-3 bg-gray-500 hover:bg-gray-700 rounded-md transition-colors duration-300 flex items-center justify-center disabled:opacity-50"
      disabled={pending}
    >
      {pending ? (
    ...
    return (
      <div className="space-y-1">
        <Component
          ...
          className={`w-full px-6 py-3 bg-gray-800/50 border ${errors ? 'border-red-500' : 'border-gray-700/30'
            } rounded-md focus:outline-none focus:ring-2 focus:ring-gray-100 text-gray-200`}
          {...(isTextArea ? { rows: 4 } : {})}
        />
  ...
  return (
    <section className="py-20">
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          {state?.error && (
            <div className="text-red-500 bg-red-100/10 p-3 rounded">
              {state.error}
            </div>
          )}
