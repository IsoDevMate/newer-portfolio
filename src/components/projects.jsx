...
export const ProjectsSection = () => {
    ...
    return (<AnimationWrapper className="py-24 px-6 bg-gray-900/50 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div variants={childVariants} className="flex flex-wrap justify-center gap-4 mb-12">
          {categories.map(category => (<button key={category.id} onClick={() => setFilter(category.id)} className={`px-4 py-2 rounded-full transition-all duration-300
                ${filter === category.id
        ...
        {/* Projects Grid */}
        <motion.div variants={childVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project, index) => (<motion.div key={project.title} variants={childVariants} className="group relative bg-gray-800/50 rounded-lg p-6 border border-gray-700/30 backdrop-blur-sm
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-4 text-gray-400">
                  {project.githubLink && (<a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="hover:text-blue-400 transition-colors">
                      <GitHub size={20}/>
                    </a>)}
                  {project.liveLink && (<a href={project.liveLink} target="_blank" rel="noopener noreferrer" className="hover:text-blue-400 transition-colors">
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {project.technologies.map(tech => (<span key={tech} className="text-xs px-2 py-1 rounded-full bg-gray-700/50 text-gray-300">
                    {tech}
                  </span>))}
              </div>

              {project.featured && (<div className="absolute top-3 right-3 text-yellow-400">
