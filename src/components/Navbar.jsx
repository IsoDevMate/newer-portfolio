...
export const Navbar = () => {
    return (<motion.nav initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="w-full py-4 px-6">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <div className="text-sm">
          <Link to="/" className="text-gray-400 hover:text-white transition-colors">
            <img src={barack} alt="Profile" className="w-8 h-8 rounded-full"/>
             
          </Link>
        </div>
        <div className="flex gap-4 items-center">
          {socialLinks.map(({ Icon, url, label }) => (<a key={label} href={url} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors" aria-label={label}>
               {label}
            </a>))}
